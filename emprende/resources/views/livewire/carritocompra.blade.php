  ...
  <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
    <div class="rounded-lg md:w-2/3">
      ...
      <div class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
        @if($item->producto)
          ...
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between sm:items-center">
            <div>
              ...
              <p class="mt-1 text-xs text-gray-700">${{number_format($item->producto->precio)}}</p>
            </div>
            <div class="mt-4 flex flex-col items-center sm:space-y-6 sm:mt-0 sm:flex sm:flex-row sm:items-center sm:space-x-6">
              ...
              <div class="flex items-center space-x-4 mt-2 sm:mt-0">
                <p class="text-sm">${{number_format($item->producto->precio * $item->cantidad)}}</p>
        ...
        @elseif($item->oferta)
          <img src="imagenes/ofertas/{{$item->oferta->imagen}}" alt="oferta-image" class="w-full rounded-lg sm:w-40" />
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between sm:items-center">
            <div>
              <h2 class="text-lg font-bold text-gray-900">{{$item->oferta->nombre}}</h2>
              <p class="mt-1 text-xs text-gray-700">${{ number_format($item->oferta->precioDescuento)}}</p>
            </div>
            <div class="mt-4 flex flex-col items-center sm:space-y-6 sm:mt-0 sm:flex sm:flex-row sm:items-center sm:space-x-6">
              <div class="flex items-center gap-1 border-gray-100">
                <button class="w-5 h-5 rounded-full border border-gray-300 cursor-pointer" wire:click="decrementCant({{ $item->id }})">
              ...
              <div class="flex items-center space-x-4 mt-2 sm:mt-0">
                <p class="text-sm">${{number_format ($item->oferta->precioDescuento * $item->cantidad) }}</p>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 cursor-pointer duration-150 hover:text-red-500" wire:click="eliminarItem({{ $item->id }})">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </div>
          ...
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between sm:items-center">
            <div>
              ...
              <p class="mt-1 text-xs text-gray-700">${{ number_format($item->servicio->precio)}}</p>
            </div>
            <div class="mt-4 flex flex-col items-center sm:space-y-6 sm:mt-0 sm:flex sm:flex-row sm:items-center sm:space-x-6">
              <div class="flex items-center gap-1 border-gray-100">
                <button class="w-5 h-5 rounded-full border border-gray-300 cursor-pointer" wire:click="decrementCant({{ $item->id }})">
              ...
              <div class="flex items-center space-x-4 mt-2 sm:mt-0">
                <p class="text-sm">${{number_format ($item->servicio->precio* $item->cantidad) }}</p>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 cursor-pointer duration-150 hover:text-red-500" wire:click="eliminarItem({{ $item->id }})">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </div>
